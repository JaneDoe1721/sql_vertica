...
select
    ...
    join (
        ...
        from wms_csharp_service_storage_all.movement_log tp_
    join (
             select
        ...
        ) ai on ai.item_id = tp_.item_id
        group by tp_.reason_id) tsk_ovh on tsk_ovh.reason_id = tp.reason_id
    join item on item.item_id = tp.item_id
where tp.supply_id = sl.supply_id
    and cast (tl.at_ AT TIME ZONE 'UTC' AT TIME ZONE 'MSK' as smalldatetime) >= '2022-01-23 08:00'
    and cast (tl.at_ AT TIME ZONE 'UTC' AT TIME ZONE 'MSK' as smalldatetime) < '2022-02-11 08:00'
group by
    u.name,
    tp.reason_id,
    tl.at_,
